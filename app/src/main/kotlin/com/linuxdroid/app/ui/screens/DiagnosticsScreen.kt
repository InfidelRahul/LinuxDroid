...
fun DiagnosticCheckRow(check: DiagnosticCheck) {
    val (icon, tint) = when (check.status) {
        ...
        DiagnosticStatus.UNKNOWN -> Icons.Default.Help to MaterialTheme.colorScheme.onSurfaceVariant
        DiagnosticStatus.NOT_APPLICABLE -> Icons.Default.Help to MaterialTheme.colorScheme.onSurfaceVariant
    }
    Row(
        modifier = Modifier
            .fillMaxWidth()
